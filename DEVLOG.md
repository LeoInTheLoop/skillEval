...
结论。此次直接复用历史 PDF V1 run：3 道 rej × 3 repeats 全部把内容分析误激活为 `pdf`。
先 dry-run 审核 340 字 Skill、9 条 trajectory 和一次调用清单，再用
`openai/qwen3.7-flash-2026-07-15` 单次生成建议，不执行 `--apply`。

真实输出把 3 个 case 聚成一个根因，逐 case 引用原文，并明确把激活边界从“用户处理 PDF
文件”收窄到结构操作、表单与精确表格抽取，排除普通阅读、总结和数据分析。人工按预先写死
的四条标准判 PASS；输入、结果哈希和 suggestion 原文已归档在
`evals/analysis/pdf_v1_suggestion_capability_v1.0.md`。没有创建 PDF V3，也没有修改 V1/V2。
