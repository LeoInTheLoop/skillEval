...
当前 OpenClaw 版本只给聚合 `toolSummary`，所以它能提供 coarse tool evidence 和
真实 workspace state diff，但不能证明逐次参数或精确顺序；这些维度会被记录为 N/A。
以后 runtime 提供事件级 tool trace 时，只需填 `RunResult.trajectory`，无需改变
case、report 或 evaluator 接口。
